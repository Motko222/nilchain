...
network_height=$(curl -s https://rpc-zero-gravity-testnet.trusted-point.com/status | jq -r .result.sync_info.latest_block_height)
...
wallet_eth=4(echo "0x$(0gchaind debug addr $(0gchaind keys show ant100 -a) | grep hex | awk '{print $3}')")
